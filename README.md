...
一個簡單的 2×2 種田網頁遊戲，為受控社課刻意保留兩類前端／URL 信任問題。請只在主辦方管理的區網與授權活動中使用，**不要把它部署到公網**。
...
- 「開始新活動（清空累計）」會把所有人的累計分數歸零並開始第 1 局，按下前會再確認一次。
- 「接續開始（保留累計）」會直接開始新的一局，保留 SQLite 中既有的累計分數。伺服器中途重開後要接回原本的活動，請用這顆。
- 每局時間到會自動結算本局分數（同時寫入 SQLite），接著進入固定休息時間。
- 休息結束後會自動開始下一局。
- 主辦方也可以提早結算本局、提早開始下一局或直接結束整場活動。
- 每局開始時，玩家會回到 100 元、0 本局分數，種子、作物與四格田地也會清空。
...
兩個刻意保留的漏洞集中在 `src/routes/farm.js`，原始碼中以 `INTENTIONALLY VULNERABLE` 註解標示。
